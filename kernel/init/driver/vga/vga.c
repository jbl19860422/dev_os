...
void _DrawBmp888(int x0, int y0, char *pBmpData, int iWidth, int iHeight)
{
	...
	for(j = iHeight-1; j >= 0; j--)
	{
		for(i = 0; i < iWidth; i++)
		{
			char *pPointPos = &pBmpData[(iHeight-1-j)*iWidth*3+i*3];
			PutPixel888(x0+i, y0+j, pPointPos[2], pPointPos[1], pPointPos[0]);	
...
void DisplayAscii(int x0, int y0, char c, char* maskbuff, RGB color)
{
	...
	for(i=0; i< 16; i++)//纵向
	{
		...
		for(j=7;j>=0;j--)//横向
		{
			if((m>>j)&0x1)//右移动j位，得到该位的0,1（显示从左到右)
			{
