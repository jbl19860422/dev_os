...
int FindFile(char *sFileName,FDDescriptor* fd)
{
	...
	int iRet = 0;
	buffer[0] = 0xaa;
  	iRet = ReadSector(1,0x7c0,(WORD)buffer,16,0x0);
	DisplayMsg("buff first word:");
	DisplayInt(*(int *)buffer);
	DisplayMsg("\r\n");
        FDDescriptor *pRootDirRec = ((VolumeDescriptor *)buffer)->RootDirRec;
        int iRootSector =*((int *)((char *)pRootDirRec+2));
	DisplayMsg("RootSector is:");
	DisplayInt(iRootSector);
	DisplayMsg("\r\n");
	ReadSector(1,0x7c0,(WORD)buffer,iRootSector,0x0);
